...
command {
  ...
  '-nargs=?',
  '-range=%',
  'CopyMatches call user#fn#copyMatches(<bang>0, <line1>, <line2>, <q-args>, 0)',
}
command {
  '-bang',
  '-nargs=?',
  '-range=%',
  'CopyLines call user#fn#copyMatches(<bang>0, <line1>, <line2>, <q-args>, 1)',
}

command { '-count', '-nargs=*', 'LaunchVimInstance  call user#fn#launchVimInstance(<q-args>)' }

command { 'SessionSave', [[lua require'user.fn'.sessionSave()]] }
command { 'SessionLoad', [[lua require'user.fn'.sessionLoad()]] }
